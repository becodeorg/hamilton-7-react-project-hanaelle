...
const WeekDay = ({data, weatherUnits}) => {
    ...
    useEffect(() => {
        ...
        ((data.temperature_2m_max + data.temperature_2m_min) / 2).toFixed(1);
    ...
    return (
        <div className="text-center p-6 rounded-md bg-white/30 shadow-md flex justify-center items-center md:flex-col ">
            {" "}
            //style: petite carre qui affiche temp, jour et emoji
            ...
            <div className="ml-6 text-4xl md:ml-0">
                {weatherEmojis && <div>{weatherEmojis}</div>} //afficher les
                emojis
            </div>
        </div>
    );
